<template>
    <navbar>
        ...
        <ul class="nav navbar-nav ml-auto">
           ...
           <span slot="button">
          <img src="static/img/avatars/432.png" class="img-avatar" alt="o">
             <span>&nbsp;</span>
          <span class="d-md-down-none">{{login_name}}</span>
          </span>
                </a>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                login_name: Cookies.get("UserName"),
                Logout_: "退出登录"
            }
        },
        components: {
        ...
        methods: {
            Logout() {
                axios.get("/proc/Logout").then(response => {//pass double
                    const data0 = response.data;
                    if (data0.ErrCode == 0) {
                        Cookies.remove('Admin-Token');
                        Cookies.remove('UserName');
                        Cookies.remove("meetId");
                        // Cookies.remove("Evaluated");
                        this.reload();
                    } else {
